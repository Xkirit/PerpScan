...
// Priority scoring function for low volume institutional activity detection
...
const InstitutionalActivity: React.FC = () => {
    ...
    try {
         ...
         const batchResults = await Promise.all(
           batch.map(async (ticker: any, batchIndex: number) => {
             ...
             if (index < 50) { // Only top 50 coins get real API data
               realOIChange = await get24hOIChange(symbol, currentOI, priority);
             }
      ...
      const currentSuspicious = [...allUsdtTickers]
        .filter(item => {
          ...
          console.log(`🔍 Checking ${item.symbol}: OI change ${oiChange.toFixed(2)}%, abnormality ${abnormality.toFixed(2)}σ, whale ${item.whaleRating}, volume category ${item.volumeCategory}`);
    ...
    oiData.forEach((coin) => {
      ...
      if (abnormality > 1.5 && coin.openInterestValue > 5000000) { // Higher threshold for stability, minimum OI value
        anomalySignals.push({
          ...
          message: `⚡ Statistical OI anomaly detected: ${abnormality.toFixed(1)}σ deviation (${coin.whaleRating?.toUpperCase()} whale)`,
          value: abnormality,
          timestamp: now,
      ...
      if (Math.abs(oiAcceleration) > coin.openInterest * 0.02 && coin.openInterestValue > 5000000) { // Lowered thresholds
        anomalySignals.push({
          ...
          message: `🎢 OI acceleration detected: ${oiAcceleration > 0 ? 'Accelerating' : 'Decelerating'} institutional flow`,
      ...
      // 4. 🔥 STEALTH ACCUMULATION (OI growing faster than price)
      ...
      if (coin.whaleRating === 'mega' || coin.whaleRating === 'large' || coin.openInterestValue > 10000000) { // Lowered threshold for better detection
        whaleSignals.push({
          ...
          severity: 'critical',
          message: `🐋 MEGA WHALE TERRITORY: $${(coin.openInterestValue / 1e9).toFixed(2)}B open interest`,
      ...
      if (fundingExtreme > 0.01) { // >0.01% is pressure (lowered threshold for better detection)
        squeezeSignals.push({
          ...
          message: `⚡ Extreme funding pressure: ${(coin.fundingRate * 100).toFixed(3)}% (${coin.fundingRate > 0 ? 'SHORTS SQUEEZED' : 'LONGS SQUEEZED'})`,
  ...
  // Determine if signal is bullish or bearish for color coding
  const getSignalSentiment = (signal: InstitutionalSignal): 'bullish' | 'bearish' | 'neutral' => {
    ...
    if (signal.type === 'oi_divergence') {
      return 'bullish'; // OI growing faster than price = accumulation = bullish
    }
    ...
    <div className="space-y-6">
      ...
      <div className="min-h-[800px] rounded-lg border-2 p-6 backdrop-blur-[2px]" style={{ backgroundColor: 'rgba(30, 63, 32, 0.1)', borderColor: '#2d5a31', border: '1px solid rgba(255, 255, 255, 0.2)', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06)' }}>
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="text-sm text-blue-700 dark:text-blue-400 bg-blue-100 dark:bg-blue-900/20 px-3 py-1 rounded-full">
              {`⏰ Next scan: ${countdown}`}
            </div>
          </div>
        </div>
        ...
        {suspiciousMovements.length > 0 ? (
          ...
          <div className="text-center py-12">
            <div className="flex justify-center mb-6">
              <RefreshCwIcon className="h-16 w-16 text-blue-500 animate-spin" />
            </div>
            ...
            <div className="w-full max-w-md mx-auto mb-4">
              <div className="bg-blue-100 dark:bg-blue-900/20 rounded-full h-3 overflow-hidden">
                <div 
          ...
          <div className="text-center py-16">
            ...
            <div className="bg-blue-100 dark:bg-blue-900/20 rounded-lg p-4 max-w-md mx-auto mb-4">
      ...
      <div className="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 p-6 rounded-lg border border-blue-200 dark:border-blue-800">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 text-sm">
          ...
          <div>
            <h4 className="font-bold text-blue-800 dark:text-blue-300 mb-3">📊 Volume Categories:</h4>
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <span className="px-1.5 py-0.5 text-xs rounded font-medium bg-green-200 text-green-800 dark:bg-green-800 dark:text-green-200">L</span>
                <span className="text-blue-700 dark:text-blue-400"><strong>Low Volume</strong> - Bottom 40% (stealth accumulation)</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="px-1.5 py-0.5 text-xs rounded font-medium bg-yellow-200 text-yellow-800 dark:bg-yellow-800 dark:text-yellow-200">M</span>
                <span className="text-blue-700 dark:text-blue-400"><strong>Medium Volume</strong> - 40-70% range</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="px-1.5 py-0.5 text-xs rounded font-medium bg-gray-200 text-gray-800 dark:bg-gray-700 dark:text-gray-200">H</span>
                <span className="text-blue-700 dark:text-blue-400"><strong>High Volume</strong> - Top 30% (retail active)</span>
              </div>
            </div>
          </div>
          ...
          {/* Whale Rating Badges */}
          <div>
            <h4 className="font-bold text-blue-800 dark:text-blue-300 mb-3">🐋 Whale Ratings:</h4>
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <span className="px-1.5 py-0.5 text-xs rounded font-medium bg-purple-200 text-purple-800 dark:bg-purple-800 dark:text-purple-200">XL</span>
                <span className="text-blue-700 dark:text-blue-400"><strong>Mega Whale</strong> - $1B+ open interest</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="px-1.5 py-0.5 text-xs rounded font-medium bg-red-200 text-red-800 dark:bg-red-800 dark:text-red-200">L</span>
                <span className="text-blue-700 dark:text-blue-400"><strong>Large Whale</strong> - $100M+ open interest</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="px-1.5 py-0.5 text-xs rounded font-medium bg-orange-200 text-orange-800 dark:bg-orange-800 dark:text-orange-200">M</span>
                <span className="text-blue-700 dark:text-blue-400"><strong>Medium Whale</strong> - $10M+ open interest</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="px-1.5 py-0.5 text-xs rounded font-medium bg-gray-200 text-gray-800 dark:bg-gray-700 dark:text-gray-200">S</span>
                <span className="text-blue-700 dark:text-blue-400"><strong>Small</strong> - Under $10M open interest</span>
              </div>
            </div>
          </div>
          ...
          {/* Special Indicators */}
          <div>
            <h4 className="font-bold text-blue-800 dark:text-blue-300 mb-3">🚀 Special Indicators:</h4>
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <span className="px-1.5 py-0.5 text-xs rounded font-medium bg-gradient-to-r from-blue-600 to-purple-600 text-white animate-pulse">🚀</span>
                <span className="text-blue-700 dark:text-blue-400"><strong>Highly Active</strong> - 2.5σ+ statistical anomaly</span>
              </div>
            </div>
          </div>
          ...
          {/* Key Metrics */}
          <div>
            <h4 className="font-bold text-blue-800 dark:text-blue-300 mb-3">📈 Key Metrics:</h4>
            <div className="space-y-1 text-blue-700 dark:text-blue-400">
              <div><strong>Priority Score:</strong> Combined institutional signal strength</div>
              <div><strong>Volume/OI:</strong> Turnover ratio (lower = accumulation)</div>
              <div><strong>Abnormality:</strong> Statistical deviation (σ = sigma)</div>
              <div><strong>Manipulation:</strong> Confidence % of institutional activity</div>
            </div>
          </div>

          {/* Detection Criteria */}
          <div>
            <h4 className="font-bold text-blue-800 dark:text-blue-300 mb-3">🎯 Detection Criteria:</h4>
            <div className="space-y-1 text-blue-700 dark:text-blue-400">
              <div><strong>OI Change:</strong> &gt;5% movement triggers detection</div>
              <div><strong>Statistical:</strong> &gt;1.0σ anomalies flagged</div>
              <div><strong>Volume/OI:</strong> &lt;3x ratio indicates stealth activity</div>
              <div><strong>Updates:</strong> Every 10 seconds, smart database replacement</div>
            </div>
          </div>

          {/* Best Signals */}
          <div>
            <h4 className="font-bold text-blue-800 dark:text-blue-300 mb-3">💡 Best Signals:</h4>
            <div className="space-y-1 text-blue-700 dark:text-blue-400">
              <div><strong>Green "L" + High Priority:</strong> Early institutional accumulation</div>
              <div><strong>Low Volume/OI + OI Growth:</strong> Stealth positioning</div>
              <div><strong>High Abnormality + Low Volume:</strong> Smart money entry</div>
              <div><strong>🚀 Badge:</strong> Extreme statistical activity</div>
            </div>
          </div>

          {/* Smart Database Logic */}
          <div>
            <h4 className="font-bold text-blue-800 dark:text-blue-300 mb-3">🧠 Smart Database Logic:</h4>
            <div className="space-y-1 text-blue-700 dark:text-blue-400">
              <div><strong>Add Mode:</strong> Database &lt;10 coins → Add all qualifying coins</div>
              <div><strong>Replace Mode:</strong> Database = 10 coins → Only replace if new coin has higher priority</div>
              <div><strong>Protection:</strong> High priority coins are never removed</div>
              <div><strong>Threshold:</strong> New coin priority must exceed min existing coin priority</div>
            </div>
          </div>
        </div>
      </div>
